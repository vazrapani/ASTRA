...
exports.analyzeQuestion = onRequest({ region: 'asia-northeast3' }, (req: any, res: any) => {
  cors(req, res, async () => {
    try {
...
// 한 장의 카드 해석 (일일 타로용)
exports.geminiInterpret = onRequest({ region: 'asia-northeast3' }, (req: any, res: any) => {
  cors(req, res, async () => {
    try {
...
// 여러 장의 카드 해석 (심층 타로용)
exports.interpretMultipleCards = onRequest({ region: 'asia-northeast3', timeoutSeconds: 300 }, (req: any, res: any) => {
  cors(req, res, async () => {
    try {
      ...
      const cardInfo = cards.map((c: any) => `${c.name}(${c.orientation})`).join(', ');
